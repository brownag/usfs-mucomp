...
getThresholds <- function(mus, varname) {
  df <- data.frame(mus=as.character(mus), 
  ...
  for(tl in threshold.list) {
    if(tl$name == varname & all(grepl(mus, pattern=tl$musym))) {
      res.max <- rep(tl$vmax, nrow(df))
      res.min <- rep(tl$vmin, nrow(df))
    }
  }      
...
thresholdSamples <- function(samples, threshold.list)  {
  res <- lapply(raster.pnames, function(n) {
    ...
    tab <- t(table(foo))
    tab <- round(tab/sum(tab),2)
    names(tab) <- c("LT","IN","GT")
    return(c(tab, unique(thresh)))
  })
  names(res) <- raster.pnames
  res <- list.to.df(res, idcol='variable')
  return(res)
